...
describe EasyAuth::Oauth2::Models::Account do
  describe 'oauth2_identities relationship' do
    before do
      ...
      @oauth2_identity_a = Oauth2IdentityA.create(:account => @user, :uid => @user.email, token: 'test')
      @oauth2_identity_b = Oauth2IdentityB.create(:account => @user, :uid => @user.email, token: 'test')
    end
